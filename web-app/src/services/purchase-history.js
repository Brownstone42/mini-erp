...
export async function fetchPurchaseHistory() {
  const result = await executeQuery(queryRef(dataConnect, 'ListPurchaseHistory', { limit: 6000, offset: 0 }))
  return (result.data?.purchaseHistoryLines || []).map((row) => ({ ...row, productCode: row.product?.productCode, productName: row.product?.productName, supplierCode: row.supplier?.supplierCode, supplierName: row.supplier?.supplierName }))
}
